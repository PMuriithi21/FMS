...
// Get this admin's own email as default recipient
$um = $db->prepare("SELECT email FROM users WHERE user_id=?");
$um->bind_param('i', $uid);
$um->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'send_email') {
    ...
    $sendMonth = trim($_POST['send_month'] ?? date('Y-m'));
    ...
    if (!$recipientEmail || !filter_var($recipientEmail, FILTER_VALIDATE_EMAIL)) {
        ...
        $stmt2->execute();
        $sendBySup = $stmt2->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt2->close();
        ...
        $stmt3->bind_param('ii', $sy, $sm);
        $stmt3->execute();
        $sendDaily = $stmt3->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt3->close();
        ...
        $stmt4->bind_param('ii', $sy, $sm);
        $stmt4->execute();
        $sendDeliv = $stmt4->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt4->close();

        $sendTotalVol = array_sum(array_column($sendByFuel, 'vol'));
        $sendTotalRev = array_sum(array_column($sendByFuel, 'rev'));
        $sendTotalDeliv = array_sum(array_column($sendDeliv, 'vol'));
        ...
        <body>
            ...
            <div class="subtitle">Comprehensive Monthly Report — <?= clean($sendMonthLabel) ?></div>

            <table class="stats">
                <tr>
                    <td><div class="stat-value"><?= number_format($sendTotalVol,0) ?> L</div><div class="stat-label">Total Sold</div></td>
                    <td><div class="stat-value">KES <?= number_format($sendTotalRev,0) ?></div><div class="stat-label">Total Revenue</div></td>
                    <td><div class="stat-value"><?= number_format($sendTotalDeliv,0) ?> L</div><div class="stat-label">Total Received</div></td>
                    <td><div class="stat-value"><?= count($sendDaily) ?></div><div class="stat-label">Active Days</div></td>
                </tr>
            </table>
        ...
        $tmpPath = sys_get_temp_dir() . '/comprehensive_report_' . uniqid() . '.pdf';
        file_put_contents($tmpPath, $pdfOutput);

        $emailBody = "
            <p>Hello,</p>
            <p>Please find attached the Comprehensive Monthly Report for <strong>" . clean($sendMonthLabel) . "</strong>.</p>
            <p><strong>Total Volume Sold:</strong> " . number_format($sendTotalVol,2) . " L<br>
            <strong>Total Revenue:</strong> KES " . number_format($sendTotalRev,2) . "<br>
            <strong>Total Volume Received:</strong> " . number_format($sendTotalDeliv,2) . " L</p>
        ...
        $sent = sendReportEmail(
            $recipientEmail,
            $recipientEmail,
            "Comprehensive Monthly Report - " . $sendMonthLabel,
            $emailBody,
            $tmpPath,
            "Comprehensive_Report_" . $sendMonth . ".pdf"
        );
...
// =====================================================
// LOAD REPORT DATA FOR PAGE DISPLAY
// =====================================================
...
<div class="card">
    ...
    <form method="GET" style="display:flex;gap:12px;align-items:center;margin-bottom:16px;flex-wrap:wrap;">
        ...
        <a href="reports_pdf.php?month=<?= urlencode($month) ?>" class="btn btn-primary btn-sm" target="_blank" style="text-decoration:none;">
            📄 Download PDF
        </a>
    </form>
