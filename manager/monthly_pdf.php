...
$activeDays = count(array_unique(array_column($daily, 'trans_date')));
...
<body>
    ...
    <div class="subtitle">Monthly Sales Report — <?= clean($monthLabel) ?></div>

    <table class="stats">
        <tr>
            <td>
                <div class="stat-value"><?= number_format($totalVol,0) ?> L</div>
                <div class="stat-label">Total Volume</div>
            </td>
            <td>
                <div class="stat-value">KES <?= number_format($totalRev,0) ?></div>
    ...
    <table class="data">
        <thead>
            <tr><th>Fuel Type</th><th>Volume Sold (L)</th><th>Revenue (KES)</th><th>Shifts</th></tr>
        </thead>
        <tbody>
            <?php if (empty($byFuel)): ?>
    ...
    <table class="data">
        <thead>
            <tr><th>Date</th><th>Fuel</th><th>Volume (L)</th><th>Revenue (KES)</th></tr>
        </thead>
        <tbody>
            <?php if (empty($daily)): ?>
            <tr><td colspan="4" style="text-align:center;color:#999;">No transactions recorded.</td></tr>
            <?php endif; ?>
